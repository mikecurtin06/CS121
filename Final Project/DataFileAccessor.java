...
public class DataFileAccessor extends FileAccessor{
    ...
    protected void processLine(String line) throws DataFileProcessingException {
        // TODO: implement this method
        try {
            String[] tempArr = line.split(",", -2);
            String time = "";
            int t1 = 0;
            int t2 = 0;

            if(tempArr.length != 3)
                throw new Exception("Incorrect line length: " + line);

            if (firstLine) {
                seriesOneName = tempArr[1];
                seriesTwoName = tempArr[2];
            } else {
                time = tempArr[0];
                t1 = Integer.parseInt(tempArr[1]);
                t2 = Integer.parseInt(tempArr[2]);
                TempData curData = new TempData(time, t1, t2);
